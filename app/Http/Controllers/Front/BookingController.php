...
class BookingController extends Controller
{
    ...
    public function review($order_id)
    {
        ...
        return view('front.transaksi.review', compact('order_details'));
    }

    public function review_action(Request $request)
    {
        for ($i = 0; $i < count($request->room_id); $i++) {
            $review = DB::table('reviews')->where('room_id', $request->room_id[$i])->where('customer_id', Auth::guard('customer')->user()->id)->first();

            if ($review) {
                Review::where('room_id', $request->room_id[$i])->where('customer_id', Auth::guard('customer')->user()->id)->update([
                    'star' => $request->star[$i],
        ...
        return redirect()->route('transaksi')->with('success', 'Review Successfully!');
    }
